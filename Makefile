...
.PHONY: help venv bootstrap test lint typecheck simulate-room leaderboard-build leaderboard-reset docker-build docker-test docker-replay webhook-serve edge-worker-install edge-worker-typecheck edge-worker-test edge-worker-dev edge-worker-deploy clean
...
edge-worker-test: edge-worker-install ## Run Cloudflare Worker unit tests
	cd edge-worker && npm test

edge-worker-dev: ## Run Cloudflare Worker locally via Wrangler
	cd edge-worker && npx wrangler dev

edge-worker-deploy: edge-worker-install ## Deploy Cloudflare Worker via Wrangler
